...
!!!!$MP PARALLEL PRIVATE(nrecv, reqr, ierr, lrecv, lsend, ineigh, ineigh_proc,  &
!!!!$MP                  jj, k, nsweep, j, jr, istart, iend, istart2, iend2, istep,  &
!!!!$MP                  isweep, statr)
...
!!$MP DO
	 Do i = 1, maxblocks_tr
            refine_neigh(i) = .False.
	 End Do
!!$MP END DO
!!$MP DO
         Do i = lnblocks+1,maxblocks_tr 
            refine(i) = .False.
	 End Do
!!$MP END DO
...
!!!!$MP PARALLEL PRIVATE(reqr, ierr, statr)
!!!!$MP DO
         Do i = 1,nrecv
            Call MPI_IRECV(kk(i),                                      & 
                           1,                                          & 
...
!!!!$MP DO PRIVATE(ineigh, ineigh_proc, idummy_array)
         Do i = lnblocks+1,lnblocks2
            If (newchild(i)) Then
               ineigh = neigh(1,j,i)
               ineigh_proc = neigh(2,j,i)
...
!$MP PARALLEL PRIVATE(nrecv, reqr, ierr, j)
!$MP DO                  
      Do i = 1,lnblocks
         Do j = 1,nfaces
...
!$MP DO PRIVATE(ipar, ipar_proc, j, ineigh)
      Do i = 1,lnblocks
