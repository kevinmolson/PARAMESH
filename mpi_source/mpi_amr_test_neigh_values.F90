...
!
        ...
        integer :: lb,iproc,k,i,isize,nrecv,lnblocks_max
...
!$OMP PARALLEL DO PRIVATE(ierror, isize)
        do iproc=1,nprocs-1
...
! Test neigh values
        ...
        nerrors = 0
        do iproc=0,nprocs-1
!!$OMP PARALLEL DO PRIVATE(jface, rem_blk, rem_pe, jr) REDUCTION(+: nerrors)
          do lb=1,glnblocks(iproc)
            do jface=1,2*ndim
