...
!-----cycle through the grid blocks on this processor
      If (lnblocks > 0) Then
!$MP PARALLEL DO PRIVATE(dx, dy, dz, jf, remote_pe, remote_block, cnewchild, &
!$MP                     idest, i_dest, i_source, tempx, k, j, i,            &
!$MP                     area11, area21, area12, area22, b11, b21, b12, b22, &
!$MP                     areay1, areay2, areaz1, areaz2, j_dest, j_source,   &
!$MP                     areax1, areax2, tempy, k_dest, k_source, tempz,     &
!$MP                     ea, eb, e1, e2)
      Do isg = 1,lnblocks
...
!-----div B check
      If (lnblocks > 0) Then
!$MP PARALLEL DO PRIVATE(divb) REDUCTION(max : divbmax)
      Do isg = 1,lnblocks
