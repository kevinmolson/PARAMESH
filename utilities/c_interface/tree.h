...
neigh = (int (*)[][*mfaces][two_tree]) c_amr_get_pointer("neigh");
(*neigh)[0][0][0] = (*neigh)[0][0][0];

int (*child)[*maxblocks_tr][*mchild][two_tree];
child = (int (*)[][*mchild][two_tree]) c_amr_get_pointer("child");
...
parent = (int (*)[][two_tree]) c_amr_get_pointer("parent");
...
bflags = (int (*)[][*mflags]) c_amr_get_pointer("bflags");
...
coord = (float (*)[][*mdim]) c_amr_get_pointer("coord");
(*coord)[0][0] = (*coord)[0][0];

float (*bsize)[*maxblocks_tr][*mdim];
bsize = (float (*)[][*mdim]) c_amr_get_pointer("bsize");
(*bsize)[0][0] = (*bsize)[0][0];

float (*bnd_box)[*maxblocks_tr][*mdim][two_tree];
bnd_box = (float (*)[][*mdim][two_tree]) c_amr_get_pointer("bnd_box");
...
level_cell_sizes = (float (*)[][*mdim]) c_amr_get_pointer("level_cell_sizes");
...
boundary_box = (float (*)[][*mdim][two_tree]) c_amr_get_pointer("boundary_box");
...
surr_blks = (int (*)[][3+2**k2d][1+2**k3d][three_tree]) c_amr_get_pointer("surr_blks");
