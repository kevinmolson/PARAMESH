...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    !!!here!!!
         ...
         dy_fxp = amr_minmod(fx(iv1,i+ihp,j+1,k)-fx(iv1,i+ihp,j,k), & 
     &                   fx(iv1,i+ihp,j,k)-fx(iv1,i+ihp,j-1,k)) ! dy(i+ihp,[j,j-1],k)
         dz_fxp = amr_minmod(fx(iv1,i+ihp,j,k+1)-fx(iv1,i+ihp,j,k), & 
     ...
     &                   fz(iv3,i,j,k+khp)-fz(iv3,i,j-1,k+khp)) ! dy(i,[j,j-1],k+khp)
         ...
         dy_fxm = amr_minmod(fx(iv1,i+ihm,j+1,k)-fx(iv1,i+ihm,j,k), & 
     &                   fx(iv1,i+ihm,j,k)-fx(iv1,i+ihm,j-1,k)) ! dy(i+ihm,[j,j-1],k)
         dz_fxm = amr_minmod(fx(iv1,i+ihm,j,k+1)-fx(iv1,i+ihm,j,k), & 
